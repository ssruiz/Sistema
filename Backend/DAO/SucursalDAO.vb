...
Public Class SucursalDAO
    ...
    Public Function getSucursales2() As DataSet
        Dim clientes As New Collection
        Dim ds As New DataSet
        Dim da As New MySqlDataAdapter
        Dim dt As New DataTable
        Try
            ...
            Dim mysql = "SELECT sucDesc, depoCod from sucursales"
    ...
    Public Function getSucursalNombre(ByVal id As String) As Sucursal
        Try
            Dim modelo As New Sucursal
            Dim con As New MySqlConnection(ConexionDB.cadenaConexionBD(Sesion.Usuario, Sesion.Password))
            con.Open()
            Dim query = "Select * from `sucursales` where `sucDesc`  = @codigo"
            Dim cmd As New MySqlCommand(query, con)
            cmd.Parameters.AddWithValue("@codigo", id)
            Dim reader = cmd.ExecuteReader()
