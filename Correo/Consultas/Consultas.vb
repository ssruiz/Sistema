...
Public Class Consultas
    ...
    Public Function getVentasM2(ByVal fecha As Date) As Double
    ...
    Public Function getCorte(ByVal fecha As Date) As Double
        Dim clientes As New Collection
        Dim ds As New DataSet
        Dim da As New MySqlDataAdapter
        Dim dt As New DataTable
    ...
    Public Function getPendientes(ByVal fecha As Date) As Double
        Dim clientes As New Collection
        Dim ds As New DataSet
        Dim da As New MySqlDataAdapter
        Dim dt As New DataTable
    ...
    Public Function getProduccion(ByVal fecha As Date) As Double
        Dim clientes As New Collection
        Dim ds As New DataSet
        Dim da As New MySqlDataAdapter
        Dim dt As New DataTable
        ...
        Try
            ...
            con = New MySqlConnection(ConexionDB.cadenaConexionBD())
            con.Open()
            Dim query = "SELECT SUM(tr.m2) FROM (SELECT dt.`ventDetSup`/ dt.`ventDetCantidad` AS m2 FROM `templado` t INNER JOIN `produccion` pr  ON t.`pCod` = pr.`pCod` INNER JOIN `ventasdet` dt ON dt.`ventDetcod` = pr.`ventDetCod` INNER JOIN `productos` p ON p.`prodCod` = dt.`prodCod` WHERE DATE_FORMAT(t.`templaSalida`, '%Y-%m-%d') = @fecha AND t.`templaEstado` = 'T') AS tr"
            Dim cmdVenta As New MySqlCommand(query, con)
            cmdVenta.Parameters.AddWithValue("@fecha", fecha)
            Dim reader = cmdVenta.ExecuteReader()
    ...
    Public Function getEfectivoDia(ByVal fecha As Date) As Double
        Dim clientes As New Collection
        Dim ds As New DataSet
        Dim da As New MySqlDataAdapter
        Dim dt As New DataTable
        ...
        Try
            ...
            con = New MySqlConnection(ConexionDB.cadenaConexionBD())
            con.Open()
            Dim query = "SELECT SUM(t.efe) FROM (SELECT p.`pagoMonto` AS efe FROM `pagos` p WHERE DATE_FORMAT(`pagoFecha`, '%Y-%m-%d') = @fecha AND p.`tpagoCod` = 1 AND `pago_estado` <> 'A') AS t;"
            Dim cmdVenta As New MySqlCommand(query, con)
            cmdVenta.Parameters.AddWithValue("@fecha", fecha)
            Dim reader = cmdVenta.ExecuteReader()
    ...
    Public Function getChequesDia(ByVal fecha As Date) As Double
        Dim clientes As New Collection
        Dim ds As New DataSet
        Dim da As New MySqlDataAdapter
        Dim dt As New DataTable
        ...
        Try
            ...
            con = New MySqlConnection(ConexionDB.cadenaConexionBD())
            con.Open()
            Dim query = "SELECT SUM(t.efe) FROM (SELECT p.`pagoMonto` AS efe FROM `pagos` p WHERE DATE_FORMAT(`pagoFecha`, '%Y-%m-%d') = @fecha AND (p.`tpagoCod` = 3 or p.`tpagoCod` = 7 or p.`tpagoCod` = 5 or p.`tpagoCod` = 6)   AND `pago_estado` <> 'A') AS t;"
            Dim cmdVenta As New MySqlCommand(query, con)
            cmdVenta.Parameters.AddWithValue("@fecha", fecha)
            Dim reader = cmdVenta.ExecuteReader()
    ...
    Public Function getReposicionesDia(ByVal fecha As Date) As Double
        Dim clientes As New Collection
        Dim ds As New DataSet
        Dim da As New MySqlDataAdapter
        Dim dt As New DataTable
        ...
        Try
            ...
            con = New MySqlConnection(ConexionDB.cadenaConexionBD())
            con.Open()
            Dim query = " SELECT SUM(t.m2) FROM (SELECT (dt.`ventDetSup` / dt.`ventDetCantidad`) AS m2 FROM `reposiciones` r INNER JOIN `reposiciondetalle` dr ON dr.`idReposicion` = r.`repoCod` INNER JOIN `produccion` p ON p.`pCod` = dr.`idProd` INNER JOIN `ventasdet` dt ON dt.`ventDetcod` = p.`ventDetCod` WHERE DATE_FORMAT(r.`repoFI`, '%Y-%m-%d') = @fecha and dr.repoTemp = 'S' ) AS t;"
            Dim cmdVenta As New MySqlCommand(query, con)
            cmdVenta.Parameters.AddWithValue("@fecha", fecha)
            Dim reader = cmdVenta.ExecuteReader()
